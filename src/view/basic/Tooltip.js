...
var Tooltip = React.createClass({
	...
	render: function(){
		...
		return (
			<div className={'rt-tooltip ' + this.state.className} data-active={this.state.active} style={this.props.style} onClick={(event)=>event.stopPropagation()}>
				{this.state.content}
			</div>
		);
	}
...
Tooltip.create = function (){
	...
	return ReactDOM.render(<Tooltip />, _dom);
}
...
Tooltip.global = true;
window.addEventListener('click', ()=>Tooltip.close(), false);
