	...
	__renderFooter: function (){
		...
		if(this.state.data&&this.state.data.length){
			if(this.state.current<this.state.total){
				return <div onClick={()=>this.loadingMore()} className="footer">点击加载更多 共 ({this.state.current}/{this.state.total}) 页 {this.state.count} 条</div>;
			}else {
				return <div className="footer">真没数据了!!! 共 ({this.state.current}/{this.state.total}) 页 {this.state.count} 条</div>;
			}
		}
	},
	render: function () {
